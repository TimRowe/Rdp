...
*/
    ...
    $('#' + this.calendarId).css('z-index', 800);
    // 初始化目标地址的元素
    ...
    $('#' + this.submitBtn).bind('click', function() {
        ...
        __method.mOpts.success({'startDate': $('#' + __method.mOpts.startDateId).val(), 
								...
								'startChosDay':$(".moraSpan_chos").val(),
								'endChosDay':$(".moraSpan_chos2").val()
								});
        return false;
    });
    // 日期选择关闭按钮的 click 事件
...
pickerDateRange.prototype.addCSS = function(isCompare, specialClass) {
	...
	if(this.mOpts.theme == 'ta'){
		//为开始结束添加特殊样式
		$('#' + this.calendarId + '_' + this.date2ymd(new Date(sDate)).join('-')).removeClass().addClass(this.mOpts.firstCss).append($("<ul class='mora_chos'><li class='mClick_chos2 moraSpan_chos' value='09:00'>上午</li><li class='aClick_chos2' value='17:00'>下午</li></ul>"));
		$('#' + this.calendarId + '_' + this.date2ymd(new Date(eDate)).join('-')).removeClass().addClass(this.mOpts.lastCss).append($("<ul class='mora_chos'><li class='mClick_chos moraSpan_chos2' value='09:00'>上午</li><li class='aClick_chos' value='17:00'>下午</li></ul>"));
	
	
		setTimeout(function(){$(".mora_chos:eq(1)").fadeOut(200)},100)
		$(".first,.last").live("mouseenter",function(){
			$(this).children('ul').fadeIn(100)
		})
			$(".first,.last").live("mouseleave",function(){
			$(this).children('ul').fadeOut(100)
		})
		//如果开始结束时间相同
		$(".mora_chos li").click(function(e){
				$(".mora_chos").fadeOut(0)
				e.stopPropagation()
			})
			$(".last .mora_chos li").click(function(){
				$(this).parent('.mora_chos').children('li').removeClass('moraSpan_chos2')
				$(this).addClass('moraSpan_chos2')
			})
			$(".first .mora_chos li").click(function(){
				$(this).parent('.mora_chos').children('li').removeClass('moraSpan_chos')
				$(this).addClass('moraSpan_chos')
			})	
...
pickerDateRange.prototype.show = function(isCompare, __method) {
	...
	$("#" + this.calendarId).css('top', pos.top + 78 + 'px');
	//第一次显示的时候，一定要初始化输入框
