...
(function ($) {
    ...
    adapters.add("remote", ["url", "type", "additionalfields"], function (options) {
        ...
        //�޸İ汾֧�ֱ������ơ�modified by LT 2016-4-29
        $.each(splitAndTrim(options.params.additionalfields || options.element.name), function (i, fieldName) {
            var paramName = appendModelPrefix(fieldName, prefix);

            if (paramName.indexOf("As_") != -1) {
                preParamName = paramName;
                paramName = preParamName.substr(preParamName.indexOf("_As_") + 4);
                elementName = preParamName.substr(0, preParamName.indexOf("_As_"))
                value.data[paramName] = function () {
                    return $(options.form).find(":input").filter("[name='" + escapeAttributeValue(elementName) + "']").val();
                };
